...
const DEFAULT_INSTRUCTION = `あなたは経験豊富な弁護士アシスタントです。この法律相談の音声を分析してください。`;

/**
 * legal_prompts からカスタム指示文を取得し、
 * JSON構造を自動付与して完全なプロンプトを構築する。
 * 管理画面で編集するのは指示文のみ。JSON構造は常にシステムが付与。
 */
async function getCustomPrompt(): Promise<string> {
  ...
  try {
    ...
    if (sb) {
      ...
      if (data?.content) instruction = data.content as string;
    }
  } catch { /* fallback */ }
  // カスタム指示文 + 固定JSON構造を結合
  return `${instruction}\n\n${JSON_SCHEMA}`;
