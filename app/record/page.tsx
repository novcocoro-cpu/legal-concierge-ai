...
function SuccessToast({ message }: { message: string }) {
  return (
    <div
      className="toast fixed bottom-20 left-1/2 -translate-x-1/2 z-50 px-4 py-2 rounded-xl text-sm font-medium shadow-xl"
  ...
  return (
    ...
    >
      ...
      >
        ...
        <button onClick={onClose} className="flex-shrink-0 opacity-80 hover:opacity-100 transition-opacity ml-1" aria-label="閉じる">
...
function DurationSelector({ selected, onChange }: { selected: number; onChange: (min: number) => void }) {
  return (
    <div className="flex flex-col items-center gap-2">
      <p style={{ color: 'var(--muted)' }} className="text-xs font-medium">録音時間</p>
      <div className="flex gap-2">
        {DURATION_OPTIONS.map((min) => (
          <button
            ...
            style={
              selected === min
                ? { background: 'var(--gold-grad)', color: '#0a1628', boxShadow: '0 2px 8px rgba(201,168,76,0.3)' }
...
function BackupBanner({
  savedAt, durationSec, onRestore, onDiscard,
  ...
  return (
    <div className="mx-4 mt-4 rounded-xl p-4 flex flex-col gap-3" style={{ background: 'var(--surface)', border: '2px solid var(--warning)' }}>
      <div className="flex items-start gap-2">
        ...
        <div>
          <p style={{ color: 'var(--text)' }} className="text-sm font-semibold">保存済みの録音があります</p>
          <p style={{ color: 'var(--muted)' }} className="text-xs mt-0.5">{timeStr} 保存（約{min}分）</p>
        </div>
      </div>
      <div className="flex gap-2">
        <button onClick={onRestore} className="flex-1 py-2 rounded-xl text-sm font-semibold" style={{ background: 'var(--accent)', color: '#0a1628' }}>文字起こしする</button>
        <button onClick={onDiscard} className="px-4 py-2 rounded-xl text-sm" style={{ background: 'var(--surface2)', color: 'var(--muted)', border: '1px solid var(--border)' }}>削除</button>
      </div>
    </div>
  );
}
...
export default function RecordPage() {
  ...
  useEffect(() => {
    fetch('/api/settings?key=default_recording_minutes')
      .then(r => r.json())
      .then(d => { const v = parseInt(d.value, 10); if (!isNaN(v) && v > 0) setLimitMin(v); })
      .catch(() => {});
  }, []);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      ...
      if (snap) await saveRecordingBackup(snap.chunks, snap.mimeType, seconds);
  ...
  const showSuccess = (message: string) => { setSuccessMsg(message); setTimeout(() => setSuccessMsg(null), 2500); };
  ...
  const autoSave = async (data: GeminiResult, dur: number) => {
    ...
    try {
      ...
      saveToHistory({
        title: data.title,
        caseName: caseNameInput || data.title,
        result: data,
        durationSec: dur,
  ...
  const handleToggleRecording = useCallback(async () => {
    if (isRecording) {
      ...
      try {
        ...
        if (!isPerm) showError(`マイクアクセスエラー: ${msg}`);
  ...
  const handleDownloadAudio = () => {
    ...
    const caseName = (caseNameInput || result?.title || '案件').replace(/[\\/:*?"<>|]/g, '_').slice(0, 30);
    ...
    return (
      <div style={{ background: 'var(--bg)', maxWidth: '480px' }} className="mx-auto min-h-screen flex items-center justify-center p-6">
        <div className="w-full flex flex-col items-center gap-5">
          <div className="w-20 h-20 rounded-full flex items-center justify-center gold-glow" style={{ background: 'var(--gold-grad)' }}>
            <span className="text-4xl">⚖️</span>
          </div>
          <div className="text-center">
            <h1 style={{ color: 'var(--accent)', fontFamily: "'Noto Serif JP', serif" }} className="text-2xl font-bold mb-2">法務コンシェルジュ</h1>
            <p style={{ color: 'var(--muted)' }} className="text-sm leading-relaxed">
              録音するだけで、相談要約・法的論点<br />対応方針・訴訟リスク評価を<br />AI が自動生成します。
            </p>
          </div>
          <div className="w-full">
            <label style={{ color: 'var(--accent)' }} className="text-xs font-medium mb-1.5 block">法律事務所名 <span style={{ color: 'var(--danger)' }}>*</span></label>
            <input type="text" placeholder="例：○○法律事務所" value={companyInput} onChange={e => setCompanyInput(e.target.value)} autoFocus
              className="w-full px-4 py-3 rounded-xl text-sm outline-none"
              style={{ background: 'var(--surface)', border: '1px solid var(--border)', color: 'var(--text)' }} />
          </div>
          <div className="w-full">
            <label style={{ color: 'var(--accent)' }} className="text-xs font-medium mb-1.5 block">弁護士名 <span style={{ color: 'var(--danger)' }}>*</span></label>
            <input type="text" placeholder="例：田中 太郎" value={nameInput} onChange={e => setNameInput(e.target.value)}
    ...
    <AppShell title="⚖️ ヒアリング録音">
      ...
      {phase === 'idle' && (
        <div className="flex flex-col items-center justify-center min-h-[60vh] gap-6 px-6">
          ...
          <div className="w-full max-w-xs">
            <label style={{ color: 'var(--accent)' }} className="text-xs font-medium mb-1.5 block">案件名（任意）</label>
            <input type="text" placeholder="例：山田太郎 離婚調停案件"
          ...
          <p style={{ color: 'var(--muted)' }} className="text-sm text-center">ボタンを押して録音を開始</p>
      ...
      {phase === 'recording' && (
        <div className="flex flex-col items-center justify-center min-h-[60vh] gap-6 px-6">
          ...
          <p style={{ color: 'var(--muted)' }} className="text-sm text-center">ボタンを押して停止・文字起こし開始</p>
          {seconds >= (limitMin * 60 - 60) && (
            <p style={{ color: 'var(--warning)' }} className="text-xs font-medium animate-pulse">まもなく自動停止します</p>
          )}
        </div>
      )}
      ...
      {phase === 'result' && result && (
        <div className="flex flex-col gap-4 p-4">
          ...
          <div className="card-legal p-4">
            <h2 style={{ color: 'var(--accent)', fontFamily: "'Noto Serif JP', serif" }} className="font-bold text-lg mb-1">{result.title}</h2>
            <p style={{ color: 'var(--muted)', fontFamily: 'JetBrains Mono, monospace' }} className="text-xs">
              {new Date().toLocaleDateString('ja-JP', { year: 'numeric', month: 'long', day: 'numeric' })}
              {caseNameInput && <span className="ml-2" style={{ color: 'var(--accent)' }}>| {caseNameInput}</span>}
            </p>
          </div>
          ...
          <div className="flex gap-2">
            <button
              ...
              style={{ background: '#1e3a5f', color: 'var(--accent)', border: '1px solid var(--accent)' }}
            >
              📄 Word
            </button>
            <button
            ...
            <button
              ...
              disabled={!audioBlob}
              className="flex-1 flex items-center justify-center gap-1.5 py-3 rounded-xl text-sm font-semibold transition-all active:scale-95 disabled:opacity-40"
              style={{ background: 'var(--gold-grad)', color: '#0a1628' }}
            >
              🎙 音声
            </button>
          ...
          <button
            onClick={() => { setPhase('idle'); setResult(null); setAudioBlob(null); setCaseNameInput(''); }}
